...
export class ClienteService {
  private urlEndPoint: string = 'http://localhost:8090/api/particiantes';
  ...
  modificar(cliente:Clientes):Observable<Clientes>{
    return this.http.put<Clientes>(`${this.urlEndPoint}/${cliente.id}`,
  ...
  listaPagina(page:string, size:string):Observable<any>{
    const params = new HttpParams()
    .set('page', page)
    .set('size', size);
    return this.http.get<any>(`${this.urlEndPoint}/pagina`,
    {params:params})
  }
}
